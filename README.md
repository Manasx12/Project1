...
- **📊 Power BI Desktop**  
  Primary platform for building interactive dashboards and analytical reports.

- **📂 Power Query**  
  Used for data cleaning, transformation, and preprocessing of raw datasets.

- **🧠 DAX (Data Analysis Expressions)**  
  Created calculated measures such as total runs, wickets, averages, win margins, and key performance indicators (KPIs).

- **🧩 Data Modeling**  
  Designed and managed relationships between matches and deliveries datasets for accurate analysis.

- **📁 Data Format**  
  Data sourced from CSV / Excel files (`matches.xlsx`, `deliveries.xlsx`).

- **🖼 Dashboard Output**  
  Final deliverables include a `.pbix` Power BI report file and a `.png` dashboard snapshot.
...
- **📂 Matches Dataset**  
  Contains match-level information such as season, teams, toss details, venue, match result, and winning team.

- **⚾ Deliveries Dataset**  
  Ball-by-ball data capturing runs scored, wickets taken, batsmen, bowlers, dismissals, and extras.

- **🔍 Data Usage**  
  Together, these datasets enable both high-level season-wise analysis and granular player-level insights across multiple IPL seasons.
